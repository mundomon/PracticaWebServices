...
if($rest->datosURL[0]!='rest') $rest->mostrarRespuesta('ERROR',400);//echo '<br>ERROr 400, no se puede procesar esta peticion';

if(count($rest->datosURL)<2) $rest->mostrarRespuesta('ERROR',400);

switch($rest->datosURL[1]){
	case 'usuario':
...
function procesarUsuario($data,$bdMysql){
	...
	//servicio rest/usuario/{id}
	else if(count($data->datosURL)==3){
		if(ctype_digit($data->datosURL[2])){
			$id=$data->datosURL[2];
	...
	//servicio rest/usuario/{id}/*
	else if(count($data->datosURL)==4){
		if(ctype_digit($data->datosURL[2])){
			$id=$data->datosURL[2];
			$propiedad=$data->datosURL[3];
